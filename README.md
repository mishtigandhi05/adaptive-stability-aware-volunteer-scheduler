...
As detailed in [`CONFIG.md`](CONFIG.md), this repository implements the experimental configurations described in the paper and independently evaluates them. Every formula, algorithm, execution level, and parameter explicitly specified in the paper is implemented as specified; unspecified parameters are documented as simulation assumptions in [`CONFIG.md`](CONFIG.md).

> **Reproducibility note**: No parameters are tuned to reproduce previously reported numerical results. Numerical values not directly specified by the methodology are treated as documented simulation assumptions. The regenerated results are the authoritative outputs of the corrected simulation.
